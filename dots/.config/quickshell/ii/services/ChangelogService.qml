...
Singleton {
    ...
    Process {
        ...
        command: [
            "bash", "-c",
            "MATCHED_DIR=\"\"; " +
            "for dir in \"" + root.repoPath + "\" \"$HOME/Downloads/ii-vynx\" \"$HOME/.local/share/ii-vynx-fork\" \"$HOME/.local/share/ii-vynx-upstream\" \"$HOME/.local/share/ii-vynx\" \"$HOME/dotfiles\"; do " +
            "  if git -C \"$dir\" rev-parse --is-inside-work-tree >/dev/null 2>&1; then " +
            "    MATCHED_DIR=\"$dir\"; " +
            "    break; " +
            "  fi; " +
            "done; " +
            "OWNER_REPO=\"vaguesyntax/ii-vynx\"; " +
            "if [ -n \"$MATCHED_DIR\" ]; then " +
            "  REMOTE_URL=$(git -C \"$MATCHED_DIR\" remote get-url origin 2>/dev/null); " +
            "  if [ -n \"$REMOTE_URL\" ]; then " +
            "    OWNER_REPO=$(echo \"$REMOTE_URL\" | sed -E 's/.*github\\.com[\\/:]//; s/\\.git$//'); " +
            "  fi; " +
            "fi; " +
            "API_URL=\"https://api.github.com/repos/$OWNER_REPO/commits?per_page=10\"; " +
            "API_DATA=$(curl -s --connect-timeout 3 --max-time 5 \"$API_URL\"); " +
