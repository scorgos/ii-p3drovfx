...
Singleton {
    ...
    function offsetAllWidgets(dx, dy) {
        let cloned = JSON.parse(JSON.stringify(root.options.background.activeWidgets || []));
        for (let i = 0; i < cloned.length; i++) {
            cloned[i].x = (cloned[i].x || 0) + dx;
            cloned[i].y = (cloned[i].y || 0) + dy;
        }
        root.options.background.activeWidgets = cloned;
    }
            ...
            property JsonObject background: JsonObject {
                ...
                property JsonObject widgets: JsonObject {
                    ...
                    property JsonObject calendar: JsonObject {
                        property string sizeMode: "2x2" // "1x1", "1x2", "2x2"
                    }
                    property JsonObject resources: JsonObject {
                        property bool vertical: false
                    }
                    property JsonObject worldClock: JsonObject {
                        property string sizeMode: "2x2" // "1x1", "2x2"
                    }
                    ...
                    property JsonObject mediaClassic: JsonObject {
                        property bool enable: false
                        property string placementStrategy: "free"
                        property real x: 249.21
                        property real y: 612.92
                    }
                    property JsonObject weatherClassic: JsonObject {
                        property bool enable: false
