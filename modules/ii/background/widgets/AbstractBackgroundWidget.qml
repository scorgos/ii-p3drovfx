...
AbstractWidget {
    ...
    property real targetX: isPreview ? 0 : (forceCenter ? centeringX : ((placementStrategy === "free" || placementStrategy === "draggable") ? ((Config.options.background.widgets.lockWidgetPositions ?? false) ? (widgetInstance !== null ? widgetInstance.x : (configEntry ? configEntry.x : 0)) : Math.max(0, Math.min(widgetInstance !== null ? widgetInstance.x : (configEntry ? configEntry.x : 0), scaledScreenWidth - width))) : calculatedX))
    property real targetY: isPreview ? 0 : (forceCenter ? centeringY : ((placementStrategy === "free" || placementStrategy === "draggable") ? ((Config.options.background.widgets.lockWidgetPositions ?? false) ? (widgetInstance !== null ? widgetInstance.y : (configEntry ? configEntry.y : 0)) : Math.max(0, Math.min(widgetInstance !== null ? widgetInstance.y : (configEntry ? configEntry.y : 0), scaledScreenHeight - height))) : calculatedY))
